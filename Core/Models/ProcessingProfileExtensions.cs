...
namespace GpuImageProcessing.Core.Models
{
	...
	public static class ProcessingProfileExtensions
	{
		...
		/// <returns><see langword="true"/> if GPU acceleration is enabled and should be used; otherwise <see langword="false"/>.</returns>
		...
		/// <exception cref="ArgumentNullException"><paramref name="profile"/> is <see langword="null"/>.</exception>
		/// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="imageCount"/>, <paramref name="imageWidth"/>, or <paramref name="imageHeight"/> is less than or equal to zero.</exception>
			...
			int imageHeight)
		{
			...
			// Apply precision reduction factor using pattern matching
		...
		/// <exception cref="ArgumentNullException"><paramref name="profile"/> or <paramref name="settings"/> is <see langword="null"/>.</exception>
