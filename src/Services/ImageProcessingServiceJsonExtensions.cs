...
public static class ImageProcessingServiceJsonExtensions
{
    ...
    public static ImageProcessingService? FromJson(string json)
    {
        ArgumentNullException.ThrowIfNull(json);

        return JsonSerializer.Deserialize<ImageProcessingService?>(json, _jsonOptions);
    }
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="json"/> is <see langword="null"/>.</exception>
    public static bool TryFromJson(string json, out ImageProcessingService? value)
    {
        ArgumentNullException.ThrowIfNull(json);

        try
        {
            value = JsonSerializer.Deserialize<ImageProcessingService?>(json, _jsonOptions);
            return true;
        }
        catch (JsonException)
        {
