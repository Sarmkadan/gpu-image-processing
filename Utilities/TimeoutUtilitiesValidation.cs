...
namespace GpuImageProcessing.Utilities
{
    ...
    /// </summary>
    public static class TimeoutUtilitiesValidation
    {
        /// <summary>
        ...
        public static IReadOnlyList<string> Validate(TimeSpan timeout)
        {
            ...
            {
                problems.Add($"Timeout cannot be negative. Given: {timeout.TotalSeconds:F1}s.");
            }
        ...
        public static IReadOnlyList<string> Validate(TimeSpan requestedTimeout, TimeSpan minimum, TimeSpan maximum)
        {
            ...
            if (minimum > maximum)
            {
                problems.Add($"Minimum timeout ({minimum.TotalSeconds:F1}s) cannot be greater than maximum timeout ({maximum.TotalSeconds:F1}s).");
            }
        ...
        public static IReadOnlyList<string> Validate(TimeSpan timeout, TimeSpan? pollInterval)
        {
            ...
            if (pollInterval.HasValue)
            {
                ...
                else if (pollInterval.Value < TimeSpan.Zero)
                {
                    problems.Add($"Poll interval cannot be negative. Given: {pollInterval.Value.TotalSeconds:F1}s.");
                }
        ...
        public static IReadOnlyList<string> Validate(TimeSpan timeout, int maxRetries, TimeSpan? initialDelay)
        {
            ...
            if (initialDelay.HasValue)
            {
                ...
                else if (initialDelay.Value < TimeSpan.Zero)
                {
                    problems.Add($"Initial delay cannot be negative. Given: {initialDelay.Value.TotalSeconds:F1}s.");
                }
            }
        ...
        {
            return Validate(timeout).Count == 0;
        }
        ...
        /// <param name="maximum">The maximum allowed timeout</param>
        /// <returns>True if valid; false otherwise</returns>
        public static bool IsValid(TimeSpan requestedTimeout, TimeSpan minimum, TimeSpan maximum)
        {
            return Validate(requestedTimeout, minimum, maximum).Count == 0;
        }
        ...
        /// <param name="pollInterval">The polling interval</param>
        /// <returns>True if valid; false otherwise</returns>
        public static bool IsValid(TimeSpan timeout, TimeSpan? pollInterval)
        {
            return Validate(timeout, pollInterval).Count == 0;
        }
        ...
        /// <param name="initialDelay">Initial delay between retries</param>
        /// <returns>True if valid; false otherwise</returns>
        public static bool IsValid(TimeSpan timeout, int maxRetries, TimeSpan? initialDelay)
        {
            return Validate(timeout, maxRetries, initialDelay).Count == 0;
        }
        ...
        /// <param name="operationName">Name of the operation being executed</param>
        /// <returns>True if valid; false otherwise</returns>
        public static bool IsValid(TimeSpan timeout, string operationName)
        {
            return Validate(timeout, operationName).Count == 0;
        }
